...
function Map(name) {
	...
	this.addMarker = function (icon, cordX, cordY, name, place) {
		...
		this.mapMarkers[markerName] = new MapMarker(markerName, icon, cordX, cordY, this, place);
	...
	this.importJSON = function (mapDataJSON) {
		...
		for (importedMarkerName in this.mapDataJSON) {
			...
			// add new marker
			this.addMarker(this.mapDataJSON[importedMarkerName].icon, this.mapDataJSON[importedMarkerName].cordX, this.mapDataJSON[importedMarkerName].cordY, importedMarkerName, this.mapDataJSON[importedMarkerName].modal.title ?? "");
...
function MapMarker(name, icon, cordX, cordY, map, place) {
	...
	this.generateMarker = function () {
		output = '<div class="KBmap__marker" data-marker-name="' + this.name + '" style="left: ' + this.cordX + '%; top: ' + this.cordY + '%"><div class="marker ' + this.place.replace(" ", "") + '" > </div></div>'
...
function MarkerModal(modalTitle, content, linkedMapMarker) {
	...
	this.clearPosition = function () {
		...
		$positionedElem.css({
			...
			'bottom': '-154px', //self.positionedElemOffsetY,
